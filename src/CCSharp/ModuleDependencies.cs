...
{
    public const string LINQ = @"--CCSharp Linq Ver 0.0.1
local linq = {}
linq.__index = linq
...
function linq:_getIterator()
    ...
    return function()
        i = i + 1
        if iterList[i] then
            return iterList[i][1], iterList[i][2]
        end
    end
end
...
function linq:toTable()
    ...
    for k, v in self:_applyPipeline() do
        if self._isArray then
            ...
            table.insert(result, v)
            --result[k] = v TODO Handle key value pairs properly without breaking sparse tables
        end
    end
    return result
end
